...
bool InitImGUI() {
	...
	::RegisterClassEx(&wc);
	HWND hwnd = ::CreateWindow(wc.lpszClassName, _T("ImGui Directx11 Editor"), WS_OVERLAPPEDWINDOW, 0, 0, 600, 600, NULL, NULL, wc.hInstance, NULL);
		...
		for (auto& renderNode : m_pRenderNodes) {
			if (ImGui::TreeNode((renderNode->GetMeshName() + std::to_string(renderNode->GetId())).c_str())) {
				...
				if (renderNode->IsCameraEnabled()) {
					...
					renderNode->SetCameraPosition(camPos);
				}
			}
		}
